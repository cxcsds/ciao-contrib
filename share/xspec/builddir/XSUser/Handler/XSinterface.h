...
int doLoad(const StringArray& rawArgs); // Aux function for PyXspec only.
...
void doNewpar(const string& input, const bool isRespPar, string& modelName, IntegerVector& paramRange);
...
// This is to provide a marker to distinguish calls to the "do<cmd>" functions
//  coming from PyXspec vs standard Xspec.  PyXspec can add this marker to
//  the end of its rawArgs array, assuming the "do<cmd>" function is set up 
//  to handle it.
const std::string FROMPYXSPEC("2fjq0ePyXspec2o4");
}
