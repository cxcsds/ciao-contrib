...
// Traits classes  
 ...
 // Policy classes
  ...
  {
     public:
        static IntegerVector getIndices(const string& modName, 
  ...
  {
     public:
        static IntegerVector getIndices(const string& sourceName, 
                        const StringArray& rangeStrs, RangePair& prevRange);
  };
...
namespace HandlerUtils
{
  ...
  bool subExpMatched (const Tcl_RegExpInfo& regExpInfo, const string& search, 
		      const IntegerVector& indices, std::map<int, std::string >& group);
  ...
  bool fillArrays(const Tcl_RegExpInfo& expInfo, const std::string& arg, 
		  std::vector<Real>& first, std::vector<Real>& second, 
		  std::vector<Real>& _default, const IntegerVector& subs);
                      ...
                      RangePair& prevRange)
{
   ...
   IntegerVector parIndices(RangeFindPolicy<T>::getIndices(groupName,
                        rangeStrs, prevRange));
