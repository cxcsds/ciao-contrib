...
int ModelExpression<Usage>::findPrecedenceLevel (string::size_type startPos, string::size_type endPos) const
{
   ...
   // Don't want to mistake any characters in table model pathnames for
   //   arithmetic operators.
   bool betweenCurls = false;
   for (size_t i=0; i<nonNestedParts.size(); ++i)
   {
      const char c = nonNestedParts[i];
      if (betweenCurls)
      {
         if (c == '}')
            betweenCurls = false;
      ...
      else
      {
         ...
         {
            level = 1;
            // Found lowest precedence.  No need to continue loop.
            break;
         }
         else if (c == '*' || c == '/')
            level = 2;
   ...
   // If nested parentheses are found, precedence must be no higher than 2.
   level = nestFound ? std::min(level, 2) : level;
