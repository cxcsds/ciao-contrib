...
template <typename T>
void Psf<T>::initialize (const std::vector<Real>& params, const IntegerVector& spectrumNums, const std::string& modelName)
{
...
void Psf<T>::calcSurfaceBrightness ()
{
  ...
  for (size_t i=0; i<nObs; ++i)
  {
     ...
     for (size_t j=0; j<nReg; ++j)
     {
        ...
        const IntegerVector& wmapPos = region->wmapPos();
  ...
  for (size_t iObs=0; iObs<nObs; ++iObs)
  {
     if (!readFactFromFile(iObs))
     {
        ...
        for (size_t iReg=0; iReg<nReg; ++iReg)
        {
           ...
           const IntegerVector& sourcePixPos = sregion->wmapPos();
	      ...
	      Real SBfact = sourceSB[iSourceIdx];
              ...
              for (size_t ipsfe=0; ipsfe<npsfe; ++ipsfe)
              {
                 ...
                 for (size_t itReg=0; itReg<nReg; ++itReg)
                 {
                    ...
                    const IntegerVector& targPixPos = tregion->wmapPos();
...
bool Psf<T>::getImageFile ()
{
  ...
  if (imageFile.length() && imageFile != FunctionUtility::NOT_A_KEY())
  {
     ...
     try
     {
        std::unique_ptr<FITS> p(new FITS(m_imageFileName));
        PHDU& primary = p->pHDU();
...
bool Psf<T>::readFactFromFile (size_t iObs)
{
   ...
   if (fileName != FunctionUtility::NOT_A_KEY())
   {
      ...
      try
      {
         std::unique_ptr<FITS> pFile(new FITS(fileName));
   ...
   if (fileName != FunctionUtility::NOT_A_KEY())
   {
      ...
      try
      {
         ...
         if (doWrite)
         {
            ...
            std::unique_ptr<FITS> pFile(new FITS(qualFileName, bitpix, nAxes, axisLengths)); 
