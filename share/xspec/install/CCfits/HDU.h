...
namespace CCfits {
  ...
  */   
  ...
  /*! \fn void HDU::readAllKeys(const std::vector<int> & keyCategories = std::vector<int>());
      ...
      returned by the keywordCategories() function, or those given by the 
      user-supplied vector of categories.  
   
      History and comment keys are read from the current HDU.
  ...
  */
  ...
  /*! \fn void HDU::copyAllKeys (const HDU* inHdu, const std::vector<int> & keyCategories = std::vector<int>());
      ...
      TYP_CKSUM_KEY category to both <i>readAllKeys()</i> and 
      <i>copyAllKeys()</i>, then the CHECKSUM keyword would be copied from 
      the old file to the new file. Similar care must be taken with metadata 
      keywords for columns.  This is more important for <i>copyAllKeys()</i>
      than for <i>readAllKeys()</i> because <i>copyAllKeys()</i> is the 
      method that is actually inserting the keywords into the new file, 
      while <i>readAllKeys()</i> is only reading in the source file's keywords.
  ...
  */
  ...
  /*! \fn static std::vector<int> HDU::keywordCategories ();
      ...
      \brief Return the default enumerated keyword categories used by readAllKeys() and copyAllKeys()

      This returns a vector of integers indicating which categories of keywords
      are the default for the readAllKeys and copyAllKeys functions.  The list 
