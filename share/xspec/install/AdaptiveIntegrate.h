...
namespace Numerics {
			     ...
			     void *p, Real& Integral, Real& IntegralError)
  {
    ...
    Real AbsResults(std::abs(out[7])*wgk[7]);
    for (size_t i=0; i<7; i++) {
      KronrodResults += wgk[i]*(out[i]+out[14-i]);
      AbsResults += wgk[i]*(std::abs(out[i])+std::abs(out[14-i]));
    }

    Real Mean = KronrodResults * 0.5;

    Real AscResults = wgk[7] * std::abs (out[7] - Mean);

    for (size_t i=0; i<7; i++) AscResults += wgk[i] * (std::abs(out[i]-Mean) + std::abs(out[14-i]-Mean));
    ...
    AbsResults *= std::abs(halfrange);
    AscResults *= std::abs(halfrange);

    IntegralError = std::abs(IntegralError);
