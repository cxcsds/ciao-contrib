...
class XSModExpTree_Iterator 
{
  ...
  public:
      ...
      RetType& operator * ();
      RetType* operator -> ();
      void childNodes (IntegerVector& children) const;
...
class XSModExpTree 
{
  ...
  public:
    ...
    typedef XSModExpTree_Iterator<T,const std::vector<T>, const T > const_iterator;
      ...
      void insert (T entry, const IntegerVector& children);
      ...
      void insertRoot (const IntegerVector& rootChildren);
      const IntegerVector& getRoot () const;
...
template <typename T, typename ValCont, typename RetType>
inline void XSModExpTree_Iterator<T,ValCont,RetType>::childNodes (IntegerVector& children) const
{
...
template <typename T>
void XSModExpTree<T>::insert (T entry, const IntegerVector& children)
{
...
template <typename T>
void XSModExpTree<T>::insertRoot (const IntegerVector& rootChildren)
{
...
template <typename T>
const IntegerVector& XSModExpTree<T>::getRoot () const
{
   if (!m_nodes.size())
   {
