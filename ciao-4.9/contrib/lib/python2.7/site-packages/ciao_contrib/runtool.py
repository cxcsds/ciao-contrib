...
parinfo['roi'] = {
    ...
    'opt': [ParSet("group","s","Make 1 srcregion per group or per individual source",'group',["group","individual","exclude"]),ParSet("targetbkg","s","Make background around all sources or just target?",'all',["all","target"]),ParValue("bkgfactor","r","Amount offset or multipled by src to be excluded by bkg",0),ParSet("bkgfunction","s","Add bkgfactor or multiply bkgfactor",'add',["add","mul"]),ParSet("bkgfactarg","s","Apply bkgfactor to all srcs in bkg or just to target?",'target',["all","target"]),ParRange("maxpix","i","Maximum number of pixels when intersecting",None,1,None),ParRange("fovres","r","Pixellation resulolution for fov check",1,0,1),ParRange("streakres","r","Pixellation resulolution for streak check",0.25,0,1),ParValue("ignore_streaksrc","b","Ignore streak region for source list ?",True),ParValue("evtfile","f","Event File (# counts per region)",None),ParValue("num_srcs","i","Number of sources output",None),ParValue("clobber","b","Remove existing output files?",False),ParRange("verbose","i","tool verbosity",0,0,0)],
    }
