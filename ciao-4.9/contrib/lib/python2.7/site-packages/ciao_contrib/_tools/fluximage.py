...
def scale_hrci_background(enband,
    ...
    try:
        ...
        add_dmh_keyword(tfile,
                        ('BKGMETH', bkgmethod, '', 'Background subtraction method'))
        add_dmh_keyword(tfile,
                        ('BKGSCALE', bkgscale, '', 'Background scaling factor'))

        # ensure a floating-point value since large integers seem to cause dmhedit to
        # fail to write out the keyword (CIAO 4.6)
        add_dmh_keyword(tfile,
                        #('BKGFACT1', bkgfact1, '', 'Numerator of BKGSCALE'))
                        ('BKGFACT1', bkgfact1 * 1.0, '', 'Numerator of BKGSCALE'))
        add_dmh_keyword(tfile,
                        #('BKGFACT2', bkgfact2, '', 'Denominator of BKGSCALE'))
                        ('BKGFACT2', bkgfact2 * 1.0, '', 'Denominator of BKGSCALE'))

        if bkgmethod == 'particle':
            add_dmh_keyword(tfile,
                            ('BKGPARAM', bkgparams, '', 'Filter used for background scaling'))
...
def run_mkexpmap(outfile, instmap, asphist, matchfile, normalize,
    ...
    with new_pfiles_environment(ardlib=False, copyuser=False, tmpdir=tmpdir):
        ...
        copy_keywords(instmap, outfile,
                      ["SPECTRUM", "WGTFILE", "ENERG_LO", "ENERG_HI", "GRATING"]
                      , tmpdir=tmpdir)
...
def run_expcorr_image(imgfile, emapfile, outfile, lookup_table,
    ...
    with new_pfiles_environment(ardlib=False, copyuser=False, tmpdir=tmpdir):
        ...
        copy_keywords(emapfile, outfile, ["SPECTRUM", "WGTFILE", "ENERG_LO", "ENERG_HI"]
                      , tmpdir=tmpdir)
...
def add_history(outputs, cmdline_pars, toolname, toolversion,
                cleanup=True):
    """add a history entry to the file header of the output file using dmhistory
    and a note about the version of the script used.
    """

    v3("Updating file headers with history record for {}\n".format(toolname))

    # This used to select which files from outputs to add history records to.
    # It now just does everything, since it is "safer" (e.g. if a new 'file type'
    # gets added), and makes the code easier.
    #
    if cleanup:
        files = outputs[1]
    else:
