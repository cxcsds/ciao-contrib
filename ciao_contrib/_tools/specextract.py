...
Merged events files should not be used for spectral analysis.") # or v1 warning?
    ...
    def _check_ppr(self,kwdict,toolname):
        ...
        """
        keys_to_check = [key for key in kwdict.keys() if key.upper().startswith("HDUCLAS")]

        for pkey in keys_to_check:
            if kwdict["HDUNAME"].lower() == "rayevent" and kwdict[pkey].lower() == "psfray":
