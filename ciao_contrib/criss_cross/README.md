...
The CrissCross helper program **clean_spec** uses the confusion tables and HETG PHA spectra as input and generates 'cleaned' spectra (and ARFs) free of identified confusion. 

**Users are encouraged to try CrissCross and clean_spec using the included jupyter notebook tutorial 'crisscross_tutorial.ipynb'. It provides an end-to-end example of running CrissCross on an HETG observation of a crowded stellar cluster and extracting a cleaned spectrum.**
...
2)  HETG-bright sources whose spectra may be confused:
    1. **subset_src_list**:  An ascii or TSV file with columns 'RA, DEC' for each source that you wish to calculate confusion for. These will be a subset of sources in the 'main_list' that are bright enough for HETG spectral extraction. These sources must match a source in the main_list. 

        **or**

    2. **single_src_pos**: A single source's RA and Dec (in degrees) can be used to calculate confusion for only that source. This source must also exist in the main_list ascii file.


3)  **evt2_file**:  An individual evt2 file or a list of evt2 files for each observation where spectra will be analyzed. 
...
1)  **confusion_output_files/table_fits_data**:  This directory holds all of the confusion fits tables for sources in the 'subset_src_list' in the format:

    a)  **confused_src_{srcID}\_consolidated_obsid_{obsi_num}.fits** is a table that includes information from flag = 'confused' or flag = 'warn' sources of confusion. Every row of this table includes a potential instance of 	confusion is used as input to the CrissCross helper function clean_spec.
...
3)  **crisscross_obsID_{obsid}_log.txt**:  A log file which includes the executed CrissCross parameter values.


#### Description of clean_spec **input** and **output**:

**clean_spec INPUT**:


1) **infile**:  An HETG PHA1 or PHA2 spectrum fits file for cleaning.

2) **conf_file**: The confusion table associated with the HETG PHA spectrum created by crisscross.

3)  **spec_root**: A root for file-naming purposes.


**clean_spec OUTPUT**:

1)  **{spec_root}\_obsid\_{obsid_num}_cleaned.pha2**:  A copy of the original PHA2 spectrum but with portions zeroed-out where confusion was identified.

2)  **{spec_root}\_obsid\_{obsid_num}\_{arm}\_{order}_cleaned.arf**:   The ARF response file associated with the arm (heg or meg) and order (-3,-2,-1,+1,+2,+3) of the spectrum where cleaning occured. The response is zeroed-out where confusion was identified and matches the cleaned.pha2 file.
