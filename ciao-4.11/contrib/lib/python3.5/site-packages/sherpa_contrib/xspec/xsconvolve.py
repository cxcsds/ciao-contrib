...
class XSgsmooth(XSConvolutionKernel):
    ...
    ----------
    SigAt6keV
        The gaussian sigma at 6 keV, in keV.
    Index
        The power of energy for the sigma variations.
...
class XSireflect(XSConvolutionKernel):
    ...
    ----------
    rel_refl
        The reflection scaling factor (1 for an isotropic source above
        the disk).
    Redshift
        The redshift of the source.
    abund
        Abundance of elements heavier than He, relative to Solar, and
        excluding iron.
    Fe_abund
...
class XSkdblur(XSConvolutionKernel):
    """The XSPEC kdblur convolution model: convolve with the laor model shape.
    ...
    ----------
    Index
        Power-law dependence of emissivity.
    Rin_G
...
class XSkdblur2(XSConvolutionKernel):
    """The XSPEC kdblur2 convolution model: convolve with the laor2 model shape.
    ...
    Incl
        The inclination angle, in degrees.
    Rbreak
        Radius at which emissivity power-law index changes
    Index1
        Power-law dependence of emissivity for radii > Rbreak

    See Also
    --------
    XSkdblur, XSkerrconv, XSrdblur
...
class XSkerrconv(XSConvolutionKernel):
    ...
    ----------
    Index
        Power-law dependence of emissivity for the inner disk.
    Index1
        Power-law dependence of emissivity for the outer disk.
    r_br_g
        Radius separating the inner and outer disk, in
        gravitational radii.
    a
    ...
    Incl
        The inclination angle, in degrees.
    Rin_ms
        The inner radius, in units of the radius of marginal stability.
    Rout_ms
        The outer radius, in units of the radius of marginal stability.
...
class XSlsmooth(XSConvolutionKernel):
    ...
    ----------
    SigAt6keV
        The lorentzian sigma at 6 keV, in keV.
    Index
        The power of energy for the sigma variations.
...
class XSrdblur(XSConvolutionKernel):
    ...
    ----------
    Betor10
        Power-law dependence of emissivity; if 10 or greater then the
        accretion-disk emissivity law is used. See [1]_ for more details.
    Rin_M
        The inner radius, in units of GM/c^2.
    Rout_M
        The outer radius, in units of GM/c^2.
...
class XSreflect(XSConvolutionKernel):
    ...
    See Also
    --------
    XSireflect, XSrfxconv, XSxilconv
...
class XSrfxconv(XSConvolutionKernel):
    ...
    def __init__(self, name='xsrfxconv'):
        self.rel_refl = Parameter(name, 'rel_refl', -1.0, min=-1.0,
                                  max=1e6, hard_min=-1.0, hard_max=1e6)
...
class XSsimpl(XSConvolutionKernel):
    ...
    FracSctr
        The scattered fraction (0 to 1).
    UpScOnly
        If greater than 0 then only up-scattering is used,
        otherwise both up- and down-scattering is included.
    ...
    def __init__(self, name='xssimpl'):
        self.Gamma = Parameter(name, 'Gamma', 2.3,
                               min=1.1, max=4.0,
                               hard_min=1.0, hard_max=5.0)
        self.FracSctr = Parameter(name, 'FracSctr', 0.05,
                                  min=0.0, max=0.4,
                                  hard_min=0.0, hard_max=1.0)
        self.UpScOnly = Parameter(name, 'UpScOnly', 1.0,
                                  min=0.0, max=100.0,
