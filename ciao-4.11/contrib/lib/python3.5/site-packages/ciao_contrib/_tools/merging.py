...
def reproject_events_task(infile, ra0, dec0, outfile,
    ...
    with rt.new_pfiles_environment(ardlib=False):
        ...
        v2("Separation={} arcsec between reference and {}".format(sepdeg*3600.0, infile))
        if sepdeg < tol:
            v3("No need to reproject {}, so copying.".format(infile))
...
def merge(process,
    ...
    for (eband, imgfile, expmap, fluxmap) in \
        ...
        imgfiles = images[eband]
        expmap_files = expmaps[eband]
        ...
        fi.run_expcorr_image(imgfile, expmap, fluxmap, ltable,
