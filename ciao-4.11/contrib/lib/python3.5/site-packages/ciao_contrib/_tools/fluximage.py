...
def name_psfmap(head, enband):
    """The name of the PSF map when evaluated for the
    given energy band.
    """

    return "{}{}.psfmap".format(head, enband.bandlabel)
...
def run_mkpsfmap(outfile, matchfile, energy, ecf,
                 message=None,
                 verbose=0,
                 tmpdir="/tmp",
                 clobber=False):
    ...
    # print("*** TODO: filter by skyfov block")
    ...
    with new_pfiles_environment(ardlib=False, copyuser=False, tmpdir=tmpdir):
        punlearn("mkpsfmap")
        args = ["infile={}".format(matchfile),
                "outfile={}[PSFMAP]".format(outfile),
...
def make_psf_maps(taskrunner, labelconv, preconditions,
    ...
    tasks = []
    for enband in enbands:
        outfile = name_psfmap(outhead, enband)

        # For now expect monochromatic energies
        enmono, wgtfile = enband.instmap_options
        if wgtfile != 'NONE':
            raise RuntimeError("Internal energy; sent weighted energy band {}".format(enband))

        task = labelconv("psfmap-{}".format(outfile))
        taskrunner.add_task(task, preconditions,
                            run_mkpsfmap,
                            outfile, matchfile, enmono, ecf,
                            tmpdir=tmpdir,
                            message=smsg,
                            verbose=verbose,
                            clobber=clobber)
...
def run_fluximage_tasks(taskrunner, labelconv,
    ...
    if want_psf:
        pmaptask = make_psf_maps(taskrunner, labelconv, [imgtask],
                                 ecf,
                                 obs,
                                 enbands,
                                 chips,
    ...
    # Add a new task to wait for both fluxtask and pmaptask
    #
    finaltask = fluxtask + '-psfmap'
    taskrunner.add_barrier(finaltask, [fluxtask, pmaptask])
    return finaltask
