...
def run_mkpsfmap(outfile, matchfile, energy, wgtfile, ecf,
    ...
    if maskfile is None:
        ...
        tmppsf = tempfile.NamedTemporaryFile(dir=tmpdir,
                                             suffix='.psfmap')
        outfile1 = tmppsf.name
        clobval = True

    with new_pfiles_environment(ardlib=False, copyuser=False, tmpdir=tmpdir):
        punlearn("mkpsfmap")
        args = ["infile={}".format(matchfile),
                "outfile={}[PSFMAP]".format(outfile1),
                "energy={}".format(enarg),
        ...
        add_defargs(args, clobval, verbose)
        ...
        if maskfile is not None:
            dmcopy("{}[sky=mask({})]".format(outfile1,
                                             maskfile),
                   outfile, clobber=clobber, verbose=verbose)
