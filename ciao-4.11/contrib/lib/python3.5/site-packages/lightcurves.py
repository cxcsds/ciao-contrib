...
def _get_default_depth():
    "Returns the default depth value from ChIPS as an integer"
    return int(pch.get_preference("default.depth"))


def _protect_chips_label(label):
    """Return a string that can be displayed by ChIPS.
...
def _add_plot(pname):
    """Add a plot with an id of pname if it doesn't exist"""

    try:
        pch.add_plot(["id", pname])
    except RuntimeError as e:
        emsg = "chips ERROR: The id {} is ".format(pname) + \
               "currently being used by another plot\n"
        if str(e) == emsg:
            pass
        else:
            raise
...
def _get_key_as_label(cr, keyname):
    """Get the value of the key from the Crate and turn it into a
    ChIPS-displayable label. Returns None if the key does not exist."""

    return _protect_chips_label(cr.get_key_value(keyname))
...
class LightCurve:
    ...
    def _add_plot_labels(self):
        ...
        label_depth = _get_default_depth() - 10
        if label_depth < 0:
            label_depth = 0

        namelabel = _protect_chips_label(self.filename.split("/")[-1])

        # pch.add_plot(["id", "labels", "style", "open"])
        lopts = ["id", "filename", "halign", 0,
                 "coordsys", pch.FRAME_NORM, "depth", label_depth]
        pch.add_label(0.05, 0.05, namelabel, lopts)
        lopts = ["id", "method", "halign", 1,
                 "coordsys", pch.FRAME_NORM, "depth", label_depth]
        pch.add_label(0.95, 0.05, _protect_chips_label(self.method), lopts)

        if 'OBJECT' in self.labels:
            lopts = ["id", "object", "halign", 0,
                     "coordsys", pch.FRAME_NORM, "depth", label_depth]
            pch.add_label(0.05, 0.95, self.labels["OBJECT"], lopts)

        if 'OBS_ID' in self.labels:
            lopts = ["id", "obsid", "halign", 1,
                     "coordsys", pch.FRAME_NORM, "depth", label_depth]
            pch.add_label(0.95, 0.95, "obsid=" + self.labels["OBS_ID"], lopts)

    def plot(self, rateaxis="y", gcol="green", erase=True):
        ...
        #
        title = "mean rate={:g} s^{{-1}}".format(self.clean_mean_rate)
        ratelabel = "Count Rate (s^{-1})"

        # Create the plot within an undo buffer so that
        #  a) a single undo call will remove the whole plot
        #  b) if there is an error during the plot then
        #     it is easy to get back to the original ChIPS state
        #
        pch.open_undo_buffer()
        try:
            if pch.info() is None:
                pch.add_frame(["id", self.method, "window.id", "lc_plot"])
            elif erase:
                pch.erase()
                pch.add_frame(["id", self.method])
            ...
            if rateaxis == "y":
                ...
                spacing = 0.0
            pch.split(2, 1, spacing)
            pch.adjust_grid_yrelsize(1, 1.2)

            pch.current_plot("rate")
            ...
            #
            pch.current_plot("rate")
            pch.set_plot_title(title)

            copts = {"line.style": "noline", "symbol.fill": True}
            ...
            if any(bti):
                ...
                copts["id"] = "bad"
                copts["symbol.style"] = "square"
                copts["symbol.size"] = 2
                pch.add_curve(x, y, copts)
            ...
            copts["id"] = "good"
            copts["symbol.style"] = "circle"
            copts["symbol.size"] = 4
            copts["symbol.color"] = gcol
            pch.add_curve(x, y, copts)

            if rateaxis == "y":
                pch.add_hline(self.clean_mean_rate,
                              ["id", "hline", "style", "dot"])

                pch.set_plot_xlabel(r"\Delta Time (ks)")
                pch.set_plot_ylabel(ratelabel)
            else:
                pch.add_vline(self.clean_mean_rate,
                              ["id", "hline", "style", "dot"])

                pch.set_xaxis(["ticklabel.visible", False])
                pch.set_plot_ylabel(r"\Delta Time (ks)")

            # plot 2: histogram
            #
            pch.current_plot("hist")
            if any(bti):
                pch.add_histogram(xlo2a, xhi2a, y2a, ["id", "everything"])

            pch.add_histogram(xlo2g, xhi2g, y2g,
                              ["id", "good", "line.color", gcol,
                               "line.thickness", 3])

            pch.add_vline(self.clean_mean_rate,
                          ["id", "vline", "style", "dot"])

            pch.limits(pch.Y_AXIS, 0, pch.AUTO)
            pch.set_plot_xlabel(ratelabel)
            pch.set_plot_ylabel("Number")
            pch.limits(pch.X_AXIS, pch.AUTO, pch.AUTO)

        except:
            pch.discard_undo_buffer()
            raise

        pch.close_undo_buffer()

    def plot_gti_limits(self, gtiname, rateaxis="y",
        ...
        try:
            ...
            if rateaxis == "y":
                xr = pch.get_plot_xrange()
                yr = pch.get_plot_yrange()
            else:
                xr = pch.get_plot_yrange()
                yr = pch.get_plot_xrange()

            yrs = [yr[0], yr[0], yr[1], yr[1]]

            startvals = self.time_to_offset(pcr.copy_colvals(cr, "start"))
            stopvals = self.time_to_offset(pcr.copy_colvals(cr, "stop"))
            xstart = np.append(startvals, xr[1])
            xend   = np.append(xr[0], stopvals)

            reg_depth = _get_default_depth() - 10
            if reg_depth < 0: reg_depth = 0

            ropts = ["fill.style", pattern, "fill.color", pcol,
                     "opacity", 1.0, "edge.style", "none",
                     "edge.color", pcol, "depth", reg_depth]

            for xs, xe in zip(xend, xstart):
                if rateaxis == "y":
                    x = [xs, xe, xe, xs]
                    y = yrs
                else:
...
def lc_clean(filename, outfile=None, mean=None, clip=3.0, sigma=None,
             ...
             plot=True, rateaxis="y", pattern="solid", gcol="green",
             pcol="red", erase=True,
             verbose=1):
    """
    ...
    if plot:
        lc.plot(rateaxis=rateaxis, gcol=gcol, erase=erase)
