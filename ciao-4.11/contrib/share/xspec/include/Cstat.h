...
void Cstat<T>::doReset(Fit* fit)
{
	...
	// diff1 is -C'/2 and diff2 is C''/2
        ...
        if (!background)
        {
	  T::specificResetCalc(S, ts, model, number(), diff1, diff2);
        }
        else
        {
           ...
           T::specificResetCalcB(S, B, Berr, ts, tb, model, number(), diff1, diff2);
        }
	++itSpec;
    }
}
...
void Cstat<T>::doReport () const
{
  ...
  if (!getSpectraForStat().empty())
  {
     ...
     tcout << T::fullName <<" = " << setw(14) << truncatedStat << " using " 
                     << bins << " PHA bins";
